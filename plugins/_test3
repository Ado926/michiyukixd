...
const handler = async (m, { conn }) => {
  ...
  try {
    const res = await fetch(`https://api.neoxr.eu/api/youtube?url=${encodeURIComponent(videoUrl)}&type=video&quality=360p&apikey=GataDios`);
    const json = await res.json();

    if (!json.status || !json.data?.url) throw "video no disponible";

    const videoBuffer = await fetch(json.data.url).then(res => res.buffer());

    await conn.sendMessage(m.chat, {
      video: videoBuffer,
      fileName: `${json.data.title || "video"}.mp4`,
      caption: toSansSerifPlain(`✦ ${json.data.title || "Video de YouTube"}`),
      mimetype: 'video/mp4'
    }, { quoted: m });
...
handler.command = new RegExp(); // Necesario para customPrefix
handler.help = ["video"];
