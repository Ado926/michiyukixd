...
async function tryDownloadVideo(url) {
  ...
  console.log("Respuesta API Neoxr completa:", JSON.stringify(json, null, 2));

  // Intentamos varias rutas para obtener la URL del video
  const videoUrl = json.result?.url || json.result?.video || json.url || json.result?.downloadUrl;

  if (videoUrl) {
    return {
      videoUrl,
      title: json.result?.title || json.title || "Video",
    };
  } else {
...
const handler = async (m, { conn }) => {
  if (
    !m.quoted ||
    !m.quoted.text ||
    !m.quoted.text.includes("乂  Y O U T U B E  -  P L A Y")
  )
    return m.reply(
      "✦ Debes responder a un mensaje que contenga '乂  Y O U T U B E  -  P L A Y'."
    );

  const linkMatch = m.quoted.text.match(ytLinkRegex);
  if (!linkMatch)
    return m.reply("✦ No se encontró un enlace de YouTube en el mensaje citado.");
