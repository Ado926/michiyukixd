...
const handler = async (m, { conn }) => {
  ...
  const linkMatch = m.quoted.text.match(/https?:\/\/(?:www\.)?(youtube\.com|youtu\.be)\/[^\s]+/);
  if (!linkMatch)
  ...
  try {
    ...
    const apiURL = `https://api.neoxr.eu/api/youtube?url=${encodeURIComponent(videoUrl)}&type=video&quality=360p&apikey=${apiKey}`;
