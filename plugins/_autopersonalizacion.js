...
const savetube = {
  ...
  download: async (link, format) => {
    if (!link || !savetube.isUrl(link)) return { status: false, code: 400, error: "Enlace inválido." };
    if (!format || !savetube.formats.includes(format)) return { status: false, code: 400, error: "Formato no válido." };
