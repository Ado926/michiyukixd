...
const handler = async (m, { conn, args, command }) => {
  ...
  try {
    // Mostrar mensaje de espera con info del video
    const search = await yts(url);
    const vid = search.videos?.[0];
    if (vid) {
      const info = `「✦」Descargando *<${vid.title}>*\n\n` +
    ...
    m.reply('❌ No se pudo obtener o enviar el audio/video. Intenta con otra canción o más tarde.');
  }
};
